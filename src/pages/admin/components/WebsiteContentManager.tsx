...
export default function WebsiteContentManager() {
  ...
  const tabs = [
    ...
    { id: 'portfolio', label: 'Portfolio', icon: 'ri-image-line' },
    { id: 'collaborations', label: 'Collaborations', icon: 'ri-team-line' },
    { id: 'contact', label: 'Contact', icon: 'ri-mail-line' },
  ];

  // Save handlers for Home page
  const handleSaveSelectedWorks = (data: any) => {
    setSelectedWorksData(data);
    setSelectedWorksModalOpen(false);
  };

  // Save handlers for About page
  const handleSaveAboutHero = (data: any) => {
    setAboutHeroData(data);
    setAboutHeroModalOpen(false);
  };

  const handleSaveAboutMain = (data: any) => {
  ...
  const handleSavePhilosophy = (data: any) => {
    setPhilosophyData(data);
    setPhilosophyModalOpen(false);
  };

  // Save handlers for Contact page
  const handleSaveContactHero = (data: any) => {
    setContactHeroData(data);
    setContactHeroModalOpen(false);
  };

  const handleSaveContactInfo = (data: any) => {
  ...
  const handleSaveCollabHero = (data: any) => {
    setCollabHeroData(data);
    setCollabHeroModalOpen(false);
  };

  const handleSaveCollabIntro = (data: any) => {
  ...
  const { loading, error, success, saveContent, saveAllContent, clearMessages } = useWebsiteContent();
  ...
  const handleSaveSection = async (page: string, section: string, data: any) => {
    const result = await saveContent(page, section, data);
    if (result) {
      ...
      setPendingChanges((prev: any) => {
        ...
        if (updated[page]) {
          ...
          if (Object.keys(updated[page]).length === 0) {
            delete updated[page];
          }
        }
        return updated;
          ...
          <div className="bg-[#1A1A1A] border border-white/10 rounded-xl p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-gray-400 text-sm">View and manage messages from your contact form</p>
              </div>
              <button 
            onClick={() => {
              // Navigate to submissions section
              const event = new CustomEvent('admin-navigate', { detail: 'submissions' });
              window.dispatchEvent(event);
            }}
            className="px-4 py-2 bg-teal hover:bg-teal-light text-white rounded-lg transition-colors cursor-pointer whitespace-nowrap text-sm"
          >
            <i className="ri-external-link-line mr-2"></i>View Submissions
          </button>
            </div>
          </div>
