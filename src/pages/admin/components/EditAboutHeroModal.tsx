...
interface EditAboutHeroModalProps {
  isOpen: boolean;
  onClose: () => void;
  initialData: {
    title: string;
...
export default function EditAboutHeroModal({ isOpen, onClose, initialData, onSave }: EditAboutHeroModalProps) {
    ...
    <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
      <div className="bg-[#1A1A1A] border border-white/10 rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        <div className="sticky top-0 bg-[#1A1A1A] border-b border-white/10 p-6 flex items-center justify-between">
          <h3 className="text-2xl font-serif text-white">Edit About Hero Section</h3>
          <button
            onClick={onClose}
